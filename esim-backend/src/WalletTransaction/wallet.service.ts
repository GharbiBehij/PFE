...
export class WalletService {
  ...
  async ReserveAmount(
    ...
    return this.walletRepository.ReserveAmount(
      userId,
      transactionId,
      amount,
      paymentMethod,
  ...
  async logLedger(
    ...
    return this.walletRepository.logLedger(
      walletId,
      amount,
      type,
      reason,
  ...
  async getSalesmanTopUpHistory(
    salesmanId: number,
    page: number,
    limit: number,
  ) {
    return this.walletRepository.getSalesmanTopUpHistory(
      salesmanId,
      page,
      limit,
    );
  ...
  async approveTopUp(topUpId: number, zonechiefId: number) {
    return this.walletRepository.approveTopUp(topUpId, zonechiefId);
  }

  async rejectTopUp(topUpId: number, zonechiefId: number) {
    return this.walletRepository.rejectTopUp(topUpId, zonechiefId);
  }
}
