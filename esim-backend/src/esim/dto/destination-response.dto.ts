...
export class DestinationPriceRangeDto {
  @ApiProperty({
    example: 500,
    description: 'Minimum destination offer price in cents',
  })
  min: number;

  @ApiProperty({
    example: 5000,
    description: 'Maximum destination offer price in cents',
  })
  max: number;
}
...
export class DestinationResponseDto {
  ...
  @ApiProperty({
    example: 500,
    description:
      'Backward-compatible alias for the minimum destination price in cents',
    required: false,
  })
  lowestPrice?: number;
}
